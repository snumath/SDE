...
\end{enumerate}
We say $\mathcal{F}$ as \textbf{$\sigma$-albegra} or \textbf{$\sigma$-field}, $A \in \mathcal{F}$ as \textbf{measurable}, and $\Omega$ as \textbf{measurable space}.
\end{defn}
...
\end{enumerate}
and $(\Omega, \mathcal{F}, \mu)$ is called a \textbf{measrue space}.
\end{defn}
...
\begin{rem}
There is a measure with no pdf: Dirac measrue
\end{rem}
...
Now, $\int fd\mu := \lim_{\|\Pi\|\rightarrow 0}\text{LS}_{\Pi}$ is called a Lebesgue Integral.
...
Partition of $x$-axis is sensitive to fluctuation and restricts to Euclidean space, while partition of $y$-axis is not.
...
$$E(X) := \int_\Omega X d\mathbb{P} \quad \text{if} \quad \int|X|d\mathbb{P} < \infty$$ 
\end{defn}
...
\item Jensen's Inequality: $\phi : \mathbb{R} \rightarrow \mathbb{R}$ convex function $\Rightarrow \phi(E(X)) \leq E(\phi(X))$ (c.f. $\phi(t) = t^2$)
\end{enumerate}
