...
In other words, $X_t = Y_t$. %(or indistinguishable, $X_t$ is a realization of $Y_t$, ...)
\end{proof}
...
$$
% and$$\sum_{n=1}^\infty \P[A_n] = \infty \Rightarrow \P[A_n\text{'s occur infinitely often}] = 1.$$
\end{thm}
...
%$$ X_{t+1}^{(n)} - X_{t}^{(n)} = \int_a^t \left[ \sigma(s,X_s^{(n)}) - \sigma(s,X_s^{(n-1)})dB_s \right] + \int_a^t \left[ f(s,X_s^{(n)}) - f(s,X_s^{(n-1)})dB_s \right]$$
$$
\begin{aligned}
...
% {\bfseries Markov Property of SDE}
...
% 현재에 어떻게 도달했냐가 아니라 현재가 어디인가만 중요함!
...
%Now $\P[X_t \leq y | \F_s] = \E[\1_{\{X_t \leq y\}} | \F_s] = \E[\1_{\{\}} | ]$, and
